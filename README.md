...
- **채용 전략 개선**: 향후 고용 시 이탈 가능성이 낮은 특징을 가진 지원자를 선별하여 안정적인 인력 확보

 **2. 근무 환경 및 정책 개선**
- **효과적인 인사·복지 정책 수립**: 근무 환경 개선을 통해 직원 만족도를 높이고, 이탈률 감소에 기여
...
- 여러 모델 중 성능이 좋았던 LightGBM 모델의 가중치를 바탕으로, 퇴사 여부에 큰 영향을 미치는 feature들을 수치와 시각화(그래프)를 통해 확인하고 이를 통해 병원에 제공할 수 있는 인사이트를 도출
  ...
  ### 신규/경력 간호사 채용 시 
  - 결혼을 한 상태이며 나이가 어느정도 있고, 병원과 가까운 곳에 거주하는 인원 채용하는 것이 유리
